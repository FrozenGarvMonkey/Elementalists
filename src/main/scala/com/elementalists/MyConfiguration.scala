...
object MyConfiguration {
  ...
  def askDevConfig(): Config = {
    ...
    localAddress = Some(InetAddress.getLocalHost())

    MyConfiguration(
      localAddress.get.getHostAddress(),
      "",
      scala.io.StdIn.readLine("Enter port: ")
    )
  }
  ...
  def askForConfig(): Config = {
    ...
    val addresses = (for (
      ...
      add <- inf.getInetAddresses.asScala
    ) yield {
      count = count + 1
      (count -> add)
    }).toMap
    ...
    if (
      scala.io.StdIn.readLine("Is running locally? y/n: ").toUpperCase != "N"
    ) {
      runLocalOnly = Some(true)
      ...
      MyConfiguration(
        scala.io.StdIn.readLine("Enter public address/domain name: "),
        localAddress.get.getHostAddress(),
        port.toString
      )
    }
  ...
  def apply(extHostName: String, intHostName: String, port: String): Config = {
    ConfigFactory.parseString(s"""
            ...
            |      "com.elementalists.protocol.JsonSerializable" = jackson-json
            |    }
            |  }
            |  remote {
            |    artery {
